...
export class PromptComponent extends React.Component<Props, State> {
  ...
  private model = monaco.editor.createModel(
    "",
    "shell",
    monaco.Uri.parse(`shell://${this.props.session.id}`)
  );
  private historyModel = monaco.editor.createModel(
    "",
    "shell-history",
    monaco.Uri.parse(`shell-history://${this.props.session.id}`)
  );
  ...
  async appendLastLArgumentOfPreviousCommand(): Promise<void> {
    ...
    if (latestHistoryRecord) {
      this.setValue(
        this.prompt.value + _.last(scan(latestHistoryRecord.command))!.value
      );
    }
  }
  ...
  private setPreviousHistoryItem(): void {
    ...
    if (currentID) {
      ...
      const previousRecord = _.findLast(
        ...
        (record) =>
          record.id < currentID && record.command !== currentRecord.command
      );
  ...
  private get promptContentNode(): HTMLDivElement {
    return this.promptContentRef.current!;
  }
