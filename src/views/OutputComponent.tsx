...
export class RowComponent extends React.Component<RowProps, {}> {
  ...
  render() {
    const charGroups = groupWhen(
      (a, b) => a.attributes === b.attributes,
      this.props.row.toArray()
    );
    const charGroupComponents = charGroups.map(
      (charGroup: Char[], index: number) => (
        <CharGroupComponent group={charGroup} key={index} />
      )
    );

    return <div className="row">{charGroupComponents}</div>;
  }
}
...
export class OutputComponent extends React.Component<Props, {}> {
  ...
  render() {
    ...
    const showCursor =
      this.props.job.status === Status.InProgress &&
      (buffer._showCursor || buffer._blinkCursor);
    ...
    const rowComponents = buffer.map((row) => (
      <RowComponent key={row.hashCode()} row={row} />
    ));
